...
class TestValidationCli:
    ...
    def test_store_entry(self, set_and_restore_data_dir, runner):
        result = runner.invoke(hello, self.store_entry_args)
        assert result.exit_code == 0

    def test_store_exit(self, set_and_restore_data_dir, runner):
        result = runner.invoke(hello, self.store_exit_args)
        assert result.exit_code == 0

    def test_upd_entry(self, set_and_restore_data_dir, runner):
        store_result = runner.invoke(hello, self.store_entry_args)
        update_result = runner.invoke(hello, self.upd_entry_args)
        assert store_result.exit_code == 0
        assert update_result.exit_code == 0

    def test_upd_exit(self, set_and_restore_data_dir, runner):
        store_result = runner.invoke(hello, self.store_exit_args)
        update_result = runner.invoke(hello, self.upd_exit_args)
        assert store_result.exit_code == 0
        assert update_result.exit_code == 0

    def test_look_day(self, set_and_restore_data_dir, runner):
        store_entry_result = runner.invoke(hello, self.store_entry_args)
        store_exit_result = runner.invoke(hello, self.store_exit_args)
        look_result = runner.invoke(hello, self.look_day_args)
    ...
    def test_look_week(self, set_and_restore_data_dir, runner):
        store_entry_result = runner.invoke(hello, self.store_entry_args)
        store_exit_result = runner.invoke(hello, self.store_exit_args)
        look_result = runner.invoke(hello, self.look_week_args)
        assert store_entry_result.exit_code == 0
        assert store_exit_result.exit_code == 0
        assert look_result.exit_code == 0
    ...
    def test_look_month(self, set_and_restore_data_dir, runner):
        store_entry_result = runner.invoke(hello, self.store_entry_args)
        store_exit_result = runner.invoke(hello, self.store_exit_args)
        look_result = runner.invoke(hello, self.look_month_args)
        assert store_entry_result.exit_code == 0
        assert store_exit_result.exit_code == 0
        assert look_result.exit_code == 0
    ...
    def test_rem_day(self, set_and_restore_data_dir, runner):
        store_entry_result = runner.invoke(hello, self.store_entry_args)
        store_exit_result = runner.invoke(hello, self.store_exit_args)
        remaining_result = runner.invoke(hello, self.rem_day_args)
        assert store_entry_result.exit_code == 0
        assert store_exit_result.exit_code == 0
    ...
    def test_rem_week(self, set_and_restore_data_dir, runner):
        store_entry_result = runner.invoke(hello, self.store_entry_args)
        store_exit_result = runner.invoke(hello, self.store_exit_args)
        remaining_result = runner.invoke(hello, self.rem_week_args)
        assert store_entry_result.exit_code == 0
        assert store_exit_result.exit_code == 0
        assert remaining_result.exit_code == 0
    ...
    def test_del_day(self, set_and_restore_data_dir, runner):
        store_entry_result = runner.invoke(hello, self.store_entry_args)
        delete_day_result = runner.invoke(hello, self.del_day_args)
        assert store_entry_result.exit_code == 0
