...
class HourMonitor:
    ...
    def store_entry_hour(self, day_date: date, entry_hour: time) -> None:
        day_to_store = Day(day_date, entry_hour=entry_hour)
        self.store_day(day_date, day_to_store)

    def check_entry_hour(self, day_date: date) -> time:
        day = self.days.get_day(day_date)
        return day.entry_hour

    def update_entry_hour(self, day_date: date, new_entry_hour: time) -> None:
    ...
    def store_exit_hour(self, day_date: date, exit_hour: time) -> None:
        day_to_store = Day(day_date, exit_hour=exit_hour)
        self.store_day(day_date, day_to_store)

    def check_exit_hour(self, day_date: date) -> time:
        day = self.days.get_day(day_date)
        return day.exit_hour

    def update_exit_hour(self, day_date: date, new_exit_hour: time) -> None:
        day_to_update = self.days.get_day(day_date)
        day_to_update.exit_hour = new_exit_hour
        self.days.update_day(day_date, day_to_update)

    def store_day(self, day_date, day_to_store):
        if self.days.day_exists(day_date):
            self.days.update_day(day_date, day_to_store)
        else:
            self.days.add_day(day_to_store)

    def check_daily_hours(self, day_date: date) -> float:
        day = self.days.get_day(day_date)
        return day.get_daily_hours()

    def check_weekly_hours(self, day_date: date) -> float:
    ...
    def check_monthly_hours(self, day_date: date) -> float:
    ...
    def check_remaining_daily_hours(self, day_date: date) -> float:
        day = self.days.get_day(day_date)
        return Config.DAILY_HOURS - day.get_daily_hours()

    def check_remaining_weekly_hours(self, day_date: date) -> float:
        weekly_hours = self.check_weekly_hours(day_date)
        return Config.WEEKLY_HOURS - weekly_hours

    def remove_day(self, day_date: date) -> None:
